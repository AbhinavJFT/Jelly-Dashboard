...
const Login = () => {
  ...
  const LoginHandler = async (values) => {
    ...
    try {
      ...
      if (
        (loginStatus === 'Admin' && role !== 'admin') ||
        (loginStatus === 'Client' && role !== 'client')
      ) {
        alert(`This username is not valid for ${loginStatus} login.`);
        setLoading(false);
        return;
      }
      ...
      if (role === 'client') {
        ...
        const company = res.data.company_data;
        
        localStorage.setItem('company', JSON.stringify(company));
        localStorage.setItem('company_id', res?.data?.company_data?.company_id);
      }

      const companyData = res.data.company_data;
      if (role === 'Client') {
        localStorage.setItem('companyId', companyData?.company_id);
      }

      setLoading(false);
      navigate('/admin/dashboard');
    } catch (error) {
      const message = error.response?.data.detail.mssg || 'An error occurred.';
      alert(message);
      setLoading(false);
    }
  };
  ...
  const toggleLoginStatus = () => {
    setLoginStatus((prev) => (prev === 'Admin' ? 'Client' : 'Admin'));
  };
        ...
        <div className="relative h-[100vh] bg-cover bg-center bg-gradient-to-b from-black to-[#343368] text-white">
          ...
          >
            <h2 className="text-3xl font-bold mt-[2rem] ml-[4.5rem]">
              {loginStatus === 'Admin' ? 'Login As Admin' : 'Login As Client'}
            </h2>
            ...
            <span
              className="w-[25rem] flex items-center justify-center text-white font-bold cursor-pointer z-10"
              onClick={toggleLoginStatus}
            >
              {loginStatus === 'Admin' ? 'Login As Client?' : 'Login As Admin?'}
            </span>
